...
| `report_writer` | Reporting (`warehouse.reporting.report_writer`) | [report_writer_implementation.md](report_writer_implementation.md) · [heuristics/Persona of The Financial Report Writer.md](heuristics/Persona%20of%20The%20Financial%20Report%20Writer.md) | **rw0–rw2 shipped** — collect + render + write + `report.build` COMMAND + CLI `warehouse report write` + artifact-backed Report writer panel on `/reporting`. **rw3 shipped** — month-end fan-out via `workflows.month_end.run_month_end_reporting_batch` + CLI `warehouse report month-end`; isolated per-household failures; audit via `report.build`. Falsifiers: `tests/test_report_writer.py`, `tests/test_dashboard.py::test_report_writer_panel_*`. Deferred: Tier-1 recon gate on external delivery, PDF/HTML, advisor approval gate, `report.publish`. |
