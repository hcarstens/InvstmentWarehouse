...
| `pm_pivot` | Whole repo (decision-plane led) | [pm_pivot_plan_implementation.md](pm_pivot_plan_implementation.md) · [research/pm_pivot_plan.md](research/pm_pivot_plan.md) · [heuristics/Mental Model of The Portfolio Manager.md](heuristics/Mental%20Model%20of%20The%20Portfolio%20Manager.md) (ℍ_Allocation — **north-star lens**; [Persona](heuristics/Persona%20of%20The%20Portfolio%20Manager.md) is the judgment companion for pv1/pv3/pv5) | **pv0 shipped** — reframe only, additive (no engine / op / frozen type). North star swapped wealth → portfolio management (the daily observe→update→allocate→check→report loop) in `CLAUDE.md` + `dashboard/status.py` + catalog landing; PM row added to the CLAUDE heuristics table. `Book`/`Portfolio` introduced as a **thin alias** of the shipped working set (`PmAdvisePayload`) with `resolve_book`/`resolve_book_from_bundle` in `warehouse.decision.pm` (no `household_id` rename — pm_pivot decision 3). IPS monitor (`policy.check`) documented as the **mandate/limit monitor** in PM vocabulary (behavior unchanged). Falsifiers: `tests/test_pm_pivot.py` (`test_book_alias_is_working_set`, `test_resolve_book_demo`, `test_resolve_book_from_bundle_hnw_rung3`, `test_status_north_star_is_pm`). **Boundary:** vocabulary + docs only; `Book` must resolve on the demo household + HNW rung 3; suite stays green. **Planned:** pv1 Black–Litterman belief engine (`decision.beliefs`, posterior μ → the po0 QP, no new market data); pv2 FIIJ ingest + `research.stats` daily loop (wires M3 guards); pv3 live PoC (FIIJ → posterior → alert); pv4 quarantine wealth machinery + fix C1; pv5 hierarchical name selection. Pre-existing C1 (red mypy in `pm_workout.py`) is **out of pv0 scope** — pv4 owns it. |
...
pm_pivot (wealth → portfolio management — reframe + one new engine)
  └─ pv0 reframe                         [shipped — north star, Book alias, mandate/limit vocab, catalog retheme]
       └─ pv1 Black–Litterman belief     [planned — posterior μ → po0 QP, no new data; critical path]
            └─ pv2 FIIJ ingest + stats   [planned — live signal views; wires M3 walk-forward guards]
                 └─ pv3 close the loop    [planned — LIVE PoC: FIIJ → posterior → allocate → alert]
                      └─ pv4 quarantine wealth + fix C1  [planned]
                           └─ pv5 hierarchical name selection  [later — after PoC]
```
...
| `warehouse.decision.ips` | Policy model, drift monitor (`policy.check`) — **a.k.a. the mandate/limit monitor** in PM vocabulary (pm_pivot pv0; same behavior), store | Generate synthetic fixtures |
...
| `warehouse.decision.pm` | `score_pm_axioms` (7-axiom narrative), `build_working_set`; `Book`/`Portfolio` alias + `resolve_book`/`resolve_book_from_bundle` (pm_pivot pv0 — PM vocabulary over the shipped working set, no new type); advisory-only composite | Mutate state; persist; import plane cores — reach specialists via `dispatch_message` only; rename `household_id` (Book is an alias, not a rename) |
