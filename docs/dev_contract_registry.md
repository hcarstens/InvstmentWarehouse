...
| `report_writer` | Reporting (`warehouse.reporting.report_writer`) | [report_writer_implementation.md](report_writer_implementation.md) · [heuristics/Persona of The Financial Report Writer.md](heuristics/Persona%20of%20The%20Financial%20Report%20Writer.md) | **rw0–rw4 shipped** — collect + render + write + `report.build` COMMAND + CLI `warehouse report write` / `warehouse report pdf` + artifact-backed Report writer panel on `/reporting` (PDF path + sha256). **rw3 shipped** — month-end fan-out via `workflows.month_end.run_month_end_reporting_batch` + CLI `warehouse report month-end`; isolated per-household failures; audit via `report.build`. **rw4 shipped** — external PDF via Pandoc with sha256 pinning; Tier-1 recon gate blocks external PDF when firm-wide breaks open. Falsifiers: `tests/test_report_writer.py`, `tests/test_dashboard.py::test_report_writer_panel_*`. Deferred: advisor approval gate, `report.publish`, Quarto upgrade. |
