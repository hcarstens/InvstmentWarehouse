...
**Status:** pa0–pa2 **shipped** (pa0 attribution + residual + PM 5th leg; pa1
thesis + kill criteria; pa2 non-performing-asset flags). Analyst slice complete;
next milestone is portfolio_optimization v1.
**Date:** 2026-06-28
...
| 2026-06-28 | **pa0 shipped (Claude).** `warehouse.decision.analyst` package (`models.py`, `attribution.py`, `review.py`) implements Addendum A: `_SEC_TO_RISK` mapping that raises on unmapped class (A.1), window de-annualization of the class assumption (A.2), the `active_return` label "active return vs ex-ante class assumption" with the idiosyncratic residual held `not_computed` (A.3), MV-weighted rollup + `active_annualized` gated on the holding floor (A.5). One new atomic op `attribution.evaluate` (EVALUATE); PM nest-dispatches it as the additive 5th leg (`AdviceBundle.attribution`). Two open calibration values pinned to `analyst_config_version="2026.06"`: `analyst_min_holding_years=0.5` and residual `WARN=0.025`/`BREACH=0.06` (annualized active), calibrated against the §9 HNW fixtures — general_hnw rung-3 sleeves WARN, the founder concentrated drawdown lot BREACHes, a zero-active probe PASSes. Frozen + registered: `PositionAttribution`, `AttributionReport`, `AnalystCheckpoint`, `AnalystReview`. Falsifiers (§A.6/§6/§7): `tests/test_analyst_attribution.py`, `tests/test_analyst_review.py`, plus PM 5th-leg + correlation-threading in `tests/test_pm_workflow.py` and the residual panel in `tests/test_dashboard.py`. Dashboard: attribution residual table in the advisory section. |
| 2026-06-28 | **pa1 shipped (Claude).** `decision/analyst/thesis.py` adds the frozen, effective-dated `PositionThesis` (mechanism + nested frozen `KillCriteria`: drawdown-vs-cost, residual cap, liquidity floor, horizon — all optional, only pre-committed limits evaluated), `KillBreach` (frozen alert), a `ThesisStore` keyed `account × instrument`, and the **pure** `evaluate_kill_criteria(position, thesis, *, as_of, active_return=None) → list[KillBreach]` — ALERTS ONLY, never stages/sells (human gate; `test_kill_criteria_no_persist`). **Pre-commitment rule pinned (axiom 2):** `effective_date ≤ acquisition_date` (on-or-before; same-day allowed), else `ThesisError` — no hindsight (`test_thesis_must_predate_acquisition`). **Checkpoint-1 pin:** `score_analyst_checkpoints(report, theses)` now flips off the pa0 stub — NOT_DOCUMENTED when no reported position carries a thesis, BREACH when any documented position trips a kill criterion, else PASS (`theses` accepts a `ThesisStore` or a plain `list[PositionThesis]`). `PositionAttribution` gains an additive `liquidity_tier` so the report scores the liquidity kill report-side. **No new atomic op** — kill-criteria is a pure function, not an op (`test_no_analyst_coordinator_op` stays green); the residual cap reads `active_return` from the shipped attribution leg. **AdviceBundle untouched** — kill breaches surface analyst-side (the function, checkpoint 1, the panel), not via a new bundle field, keeping the frozen PM contract churn-free. Synthetic theses co-emitted with households (`research/synthetic/thesis_emit.py::emit_synthetic_theses`) so kill-criteria are flow-testable without a DB (§9); concentrated single-issuer holdings get the tighter drawdown floor, so `concentrated_stress` (seed 42, `validate=False`) trips a real breach. Dashboard: a new **kill-criteria watch** panel in `dashboard/render_analyst.py` (+ `analyst_data.py`), wired into `server.py` — chose a dedicated `render_analyst.py` (pa2's NPA panel lands there too) over extending `render_advisory.py`. New emit-side calibration pinned to `analyst_config_version="2026.06"`: `analyst_kill_drawdown_pct=-0.30`, `analyst_kill_concentrated_drawdown_pct=-0.10`, `analyst_kill_residual_cap=0.10`, `analyst_kill_min_liquidity_tier=4`. Frozen + registered: `KillCriteria`, `PositionThesis`, `KillBreach`. Falsifiers: `tests/test_analyst_thesis.py` (§7) + kill-criteria-watch assertions in `tests/test_dashboard.py`. Full suite green (294). |
| 2026-06-28 | **pa2 shipped (Claude).** `decision/analyst/npa.py` adds the **pure, advisory** `flag_non_performing(positions, alts, ips, *, as_of, …) → NpaFlags` — reason-coded NPA flags, ALERTS ONLY: no op (no coordinator; `test_no_analyst_coordinator_op` stays green), no session, no optimizer mutation, no staged trade (`test_npa_no_persist`); v0 flags feed the dashboard + approval gate only (open question #13 resolved v0). Four version-pinned rules: **sustained drawdown vs cost** (lot below `analyst_npa_drawdown_pct=-0.10` *and* held past `analyst_npa_sustained_years=1.0` — a fresh dip is not a non-performer), **stale alt mark** (`last_mark_date` older than `analyst_stale_mark_days=180`), **missed capital call** (a scheduled call due on/before `as_of` with capital still unfunded — v0 heuristic since `SyntheticAltCall` has no funded flag), **IPS liquidity breach** (manifest-level, reuses `liquid_tier_nav_share` vs `ips.liquidity_tier_min_pct`). Walk-forward safe: a future acquisition or future mark raises `NpaError`. Alts consumed via a structural `AltHoldingLike`/`ScheduledCallLike` `Protocol` (satisfied by `SyntheticAltHolding`) so production decision code doesn't hard-import the synthetic fixtures package; the DB `AlternativeHoldingView` lacks `unfunded_capital`/`scheduled_calls`/`liquidity_tier` and will satisfy the protocol once enriched. Frozen + registered: `NpaFlag`, `NpaFlags` (+ `NpaReason`/`NpaSubject` enums). Dashboard: a dedicated **NPA panel** in `dashboard/render_analyst.py` (+ `npa_data.py`), wired into `server.py`, registered live in `dashboard/phases.py`; driven off an in-process `founder_executive` rung-4 (seed 11, `validate=False`) household that exercises drawdown + stale-mark + missed-call in one book (§9; liquidity-breach covered by a crafted-position unit test since real fixtures are all tier-1). Falsifiers: `tests/test_analyst_npa.py` (§7). Full suite green (315). |
