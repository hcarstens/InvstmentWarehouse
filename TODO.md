...
- [ ] **Unlocks → Portfolio Optimization v1** — **in progress** (plan: [`docs/portfolio_optimization_implementation.md`](docs/portfolio_optimization_implementation.md)). po0 = constrained mean-variance QP in sleeve-weight space on the risk plane's real Σ, pure + advisory (target w\* + Δw + risk contributions behind `optimizer.propose`, no new op, no trades staged); po1 turnover/TLH overlay, po2 scenario-robust stress, po3 lot-discrete MIQP (documented upgrade path). The hard problem; only as good as the analyst signal feeding it.
