...
def test_evaluate_risk_request_returns_manifest() -> None:
    ...
    pct_sum = sum(float(row["pct_variance_contribution"]) for row in level_2["by_class"])
    ...
    assert any(row["native_unit"] == RiskUnitType.DURATION_YEARS for row in level_3)

    level_4 = result["level_4_stress"]["scenarios"]
    assert {s["name"] for s in level_4} == {"2008_liquidity", "2020_pandemic", "2022_inflation"}
...
def test_duration_buckets_and_mismatch() -> None:
    ...
    portfolio = AssetPortfolio.model_validate(_sample_request()["asset_portfolio"])
    states = build_sleeve_states(portfolio.allocations)
    by_class = evaluate_class_contributions(states, portfolio_covariance(states))
...
def test_fingerprint_stable_and_changes_with_notional() -> None:
    ...
    assert portfolio_fingerprint(portfolio, horizon, notional_usd=Decimal("1000000")) != base


def test_stress_scenarios_are_linear_sleeve_sum() -> None:
    portfolio = AssetPortfolio.model_validate(_sample_request()["asset_portfolio"])
    stress = evaluate_stress(portfolio.allocations, notional_usd=None, mark_source="model_prior")
...
def test_risk_api_http_post_and_get() -> None:
    ...
    try:
        ...
        assert float(report["level_1_portfolio"]["annualized_volatility"]["value"]) > 0
    finally:
        server.shutdown()
