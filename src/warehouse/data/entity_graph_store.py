...
def load_entity_graph(
    ...
    if household_id:
        ...
        entity_ids = core_ids | {r.source_id for r in rel_rows} | {r.target_id for r in rel_rows}
        entity_rows = session.scalars(
            select(EntityRow).where(EntityRow.entity_id.in_(entity_ids)).order_by(EntityRow.entity_type)
        ).all()
        rel_rows = session.scalars(
            select(EntityRelationshipRow).where(
        ...
        entity_rows = session.scalars(select(EntityRow).order_by(EntityRow.entity_type)).all()
